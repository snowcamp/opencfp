...
db_exists() {
    db=$(mysql -h$DB_PORT_3306_TCP_ADDR -uroot -p$MYSQL_ROOT_PASSWORD -e "show databases like 'cfp'")
    if [ -z "$db" ]; then
	false
    else
	true
...
create_db() {
    if ! db_exists; then
	mysql -h$DB_PORT_3306_TCP_ADDR -uroot -p$MYSQL_ROOT_PASSWORD -e "create database cfp"
    fi
}
...
update_configuration_files() {
    ...
    sed -i "s%pass: ''%pass: $MYSQL_ROOT_PASSWORD%" phinx.yml
}
